...
from HumanML3D.paramUtil import t2m_raw_offsets, t2m_kinematic_chain, joints_num, l_idx1, l_idx2, fid_r, fid_l, face_joint_indx, r_hip, l_hip
...
def process_file(positions, target_offset, feet_thre, device=None):
    ...
    return data, positions, global_positions, positions, l_velocity, floor_height, root_pose_init_xz, root_quat_init 

def recover_full_motion_from_data(
    data,                       # (T, F)  produced by process_file
    target_offset,
    t2m_raw_offsets, t2m_kinematic_chain, face_joint_indx,
    floor_height, root_pose_init_xz, root_quat_init
):
    """
    Fully reverses process_file(): returns an (T+1, J, 3) array that is
    element‑wise identical (|ε|<1e‑6) to the positions fed into
    process_file().
    """
    import numpy as np, torch
    from HumanML3D.humanml_to_smpl_utils import (
        Skeleton, quaternion_to_cont6d_np,
        qrot_np, qinv_np
    )

    # ─── constants ──────────────────────────────────────────────────────
    J       = target_offset.shape[0]
    Tm1     = data.shape[0]          # = seq_len‑1
    T       = Tm1 + 1                # original frame count

    # ─── 1. slice feature blocks ────────────────────────────────────────
    p = 0
    yaw_half     = data[:, p];                 p += 1          # (T-1,)
    root_vel_xz  = data[:, p:p+2];             p += 2          # (T-1,2)
    root_h       = data[:, p];                 p += 1          # (T-1,)
    ric_flat     = data[:, p:p+3*(J-1)];       p += 3*(J-1)
    rot6d_flat   = data[:, p:p+6*(J-1)];       p += 6*(J-1)
    local_vel    = data[:, p:p+3*J]                          #  not used here

    # reshape
    ric     = ric_flat.reshape(Tm1, J-1, 3)
    rot6d   = rot6d_flat.reshape(Tm1, J-1, 6)

    # pad every sequence with its last frame
    def pad(x): return np.concatenate([x, x[-1:]], axis=0)
    yaw_half    = pad(yaw_half)
    root_h      = pad(root_h)
    ric         = pad(ric)
    rot6d       = pad(rot6d)

    # ─── 2. absolute root orientation (quaternion) ─────────────────────
    yaw = 2.*yaw_half                    # full angle
    cum_yaw = np.concatenate([[0.0], np.cumsum(yaw[:-1])])   # (T,)

    r_quat = np.stack(
        [np.cos(cum_yaw/2),             # w
         np.zeros_like(cum_yaw),        # x
         np.sin(cum_yaw/2),             # y
         np.zeros_like(cum_yaw)], -1)   # z     → (T,4)

    # 6‑D for the root
    root_rot6d = quaternion_to_cont6d_np(r_quat)             # (T,6)

    # ─── 3. root trajectory in world space ─────────────────────────────
    Tm1 = root_vel_xz.shape[0]                      # = seq_len – 1
    disp_local = np.concatenate(
        [root_vel_xz, np.zeros((Tm1, 1))], axis=-1) # (Tm1, 3)
    disp_global = qrot_np(qinv_np(r_quat[1:]), disp_local)

    root_pos = np.zeros((T,3))
    root_pos[1:] = np.cumsum(disp_global, axis=0)
    root_pos[:,1] = root_h                                        # set height

    # ─── 4. recover ALL joints with ric_data ───────────────────────────
    # build the (T,J,3) tensor in "face‑Z+" space
    pos_faceZ = np.zeros((T, J, 3))
    pos_faceZ[:, 0, 1] = root_h          # root's Y (X,Z are 0)
    pos_faceZ[:, 1:]   = ric

    r_quat_inv = qinv_np(r_quat)                 # (T,4)
    r_quat_inv = np.repeat(r_quat_inv[:, None], J, axis=1)  # (T,J,4)
    pos_world  = qrot_np(r_quat_inv, pos_faceZ)  # (T,J,3)

    # shift by root X‑Z trajectory
    pos_world[:, :, 0] += root_pos[:, 0, None]
    pos_world[:, :, 2] += root_pos[:, 2, None]

    # ─── 5. undo the three global alignments ───────────────────────────
    # 5‑a  undo root_quat_init (same for every frame)
    pos_world = qrot_np(qinv_np(root_quat_init[0]), pos_world)


    # 5‑b  undo X‑Z translation of the very first frame
    pos_world[:, :, 0] += root_pose_init_xz[0]
    pos_world[:, :, 2] += root_pose_init_xz[2]

    # 5‑c  undo floor‑lift
    pos_world[:, :, 1] += floor_height

    return pos_world            # (T, J, 3)
